package videocapture.com.videocapturedemo.videocompressor;
...
import videocapture.com.videocapturedemo.VideoCaptureActivity;
...
public class MediaController {
    ...
    public static class VideoConvertRunnable implements Runnable {
        ...
        @Override
        public void run() {
            MediaController.getInstance(mContext).convertVideo(videoPath);
        }
    }
...
//        int resultHeight = 360;
        int resultWidth = Integer.parseInt(width);
...
//        File compressedDir  = new File(mContext.getFilesDir().getAbsolutePath() + "/Video" + Config.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR);
        File compressedDir  = new File(Environment.getExternalStorageDirectory() + "/Video" + Config.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR);
...
//                + File.separator + Config.VIDEO_COMPRESSOR_APPLICATION_DIR_NAME + Config.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR);
        ...
        if (new File(compressedDir, VideoCaptureActivity.videoFileName + Config.VIDEO_FILE_FORMAT).exists()){
            new File(compressedDir, VideoCaptureActivity.videoFileName + Config.VIDEO_FILE_FORMAT).delete();
        }
        cacheFile = new File(compressedDir, VideoCaptureActivity.videoFileName + Config.VIDEO_FILE_FORMAT);
